...
class Coin {
    ...
    init(size: CGSize) {
        self.node = SKSpriteNode(imageNamed: "coin0")
        self.node.size = CGSize(width: size.height/25, height: size.height/25)
        let textures = [SKTexture(imageNamed: "coin0"), SKTexture(imageNamed: "coin1"), SKTexture(imageNamed: "coin2"), SKTexture(imageNamed: "coin3"), SKTexture(imageNamed: "coin4"), SKTexture(imageNamed: "coin5"), SKTexture(imageNamed: "coin6"), SKTexture(imageNamed: "coin7")]
        ...
        node.physicsBody?.categoryBitMask = PhysicsCategory.coins
    ...
    private func addBlinkingAndRemovalAction() {
        ...
        let blinkOn = SKAction.colorize(with: .white, colorBlendFactor: 1, duration: 0.2)
        let blinkOff = SKAction.colorize(with: .clear, colorBlendFactor: 0.0, duration: 0.2)
        let blinkSequence = SKAction.sequence([blinkOff, blinkOn])
